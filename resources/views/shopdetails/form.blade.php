...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'shopdetails?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> Shop Details</legend>
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/uploads/images/shop") !!}
						</div>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('shopdetails/comboselect?filter=status:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("shopdetails/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
